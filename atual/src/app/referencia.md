...
[ ] Inercia: Visualizador de sólidos com momento de inércia
    ...
    Exemplo de uso básico (com sólidos prontos):
        ```tsx
        "use client"
        ...
        export default function Page() {
          return (
            <div>
              <h1>Exercício de Momento de Inércia</h1>
    ...
    Exemplo de uso com controle externo (sem botão flutuante):
        ```tsx
        "use client"
    ...
    Para criar sólidos customizados:
        ```tsx
        ...
        Momento de inércia de um cubo de lado $L$ e massa $M$:
    ...
    Recursos da interface:
        - 🔍 Busca inteligente (normaliza acentos automaticamente)
        - ◀️ ▶️ Navegação por setas (teclado e botões)
        - 🖼️ Miniaturas clicáveis (3-5 sólidos visíveis)
        - 2️⃣ 3️⃣ Alternância 2D/3D
        - 📱 Tabs no mobile (Visualização | Detalhes)
        - 🖥️ Side-by-side no desktop (3D | Markdown)
        - 📊 Paginação (X/Y sólidos)
        - 🎨 Suporte a tema claro/escuro
        - ♻️ Ordenação alfabética ou por inserção

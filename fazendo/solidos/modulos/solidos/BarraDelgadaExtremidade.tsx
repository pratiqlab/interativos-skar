...
/** Barra delgada (cilindro fino e longo) com eixo pela extremidade */
function BarraDelgadaExtremidadeMesh() {
  const comprimento = 3
  const raio = 0.15
  
  return (
    <SolidView size={380}>
      {/* Barra cilíndrica deslocada para eixo na extremidade esquerda com cores distintas */}
      <mesh position={[comprimento / 2, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
      ...
      {/* Eixo de rotação (linha vertical pela extremidade esquerda) */}
      <Line
...
const TEXTO_BARRA_EXTREMIDADE = `# Barra Delgada

## Eixo pela Extremidade

Momento de inércia de uma barra delgada de massa $M$ e comprimento $L$, com o eixo de rotação passando por **uma extremidade** e perpendicular ao comprimento:
...
export function SolidoBarraDelgadaExtremidade() {
  return (
    <Solido
      previewUrl="https://i.postimg.cc/5NLK8Gr6/Chat-GPT-Image-29-de-nov-de-2025-21-46-25.png"
      name="Barra Delgada (extremidade)"
      description="Barra fina com eixo de rotação passando por uma extremidade. Ex: porta, remo."
    >
      <Solid3D><BarraDelgadaExtremidadeMesh /></Solid3D>
      <SolidText>{TEXTO_BARRA_EXTREMIDADE}</SolidText>
    </Solido>
  )
}
